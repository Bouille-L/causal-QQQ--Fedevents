...
Collect U.S. macro event-style data from FRED over a fixed window and produce a strictly timed, unified event file for downstream use.
  ...
  fred_datacollection.py : the objective of this component is to build a single event-style dataset of U.S. macro releases from FRED over a hard-wired date window (2011-06-17 to 2025-06-16) using official release calendar and pull first release (as published) event value,  
  ...
  Layer2_format_events.py : this component is responsible of unifeing fred macro evenst dataset we collected from component "fred_datacollection.py " and make sure is striclty timed  using policy bellow : 
